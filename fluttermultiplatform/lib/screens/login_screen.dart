...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _handleSubmit() async {
    ...
    if (_passCtrl.text.length < 6) {
      setState(() {
        _error = 'Password minimum 6 karakter';
        _loading = false;
      });
      return;
    }
    try {
      ...
      if (mounted) {
        toastification.show(context: context, title: Text(context.read<AppLocalizations>().t('auth.welcome_back')), type: ToastificationType.success, autoCloseDuration: const Duration(seconds: 3));
              ...
              child: ConstrainedBox(
                ...
                child: Column(
                  children: [
                    ...
                    Text(loc.t('auth.welcome_back'), style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 4),
                    Text(loc.t('auth.signin_sub'), style: TextStyle(color: CronosColors.gray500, fontSize: 14)),
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 6),
                            TextField(controller: _passCtrl, obscureText: !_showPassword, decoration: InputDecoration(prefixIcon: const Icon(Icons.lock_outline, size: 20), hintText: '••••••',
                            ...
                            ElevatedButton(onPressed: _loading ? null : _handleSubmit, style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 16)),
                              child: Text(_loading ? loc.t('auth.signing_in') : loc.t('auth.sign_in'))),
                            const SizedBox(height: 20),
                            Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                              Text('${loc.t('auth.no_account')} ', style: TextStyle(color: CronosColors.gray500, fontSize: 13)),
                              GestureDetector(onTap: () => context.go('/register'), child: Text(loc.t('nav.register'), style: const TextStyle(color: CronosColors.primary600, fontWeight: FontWeight.w600, fontSize: 13))),
                            ]),
                          ],
                        ),
                      ),
